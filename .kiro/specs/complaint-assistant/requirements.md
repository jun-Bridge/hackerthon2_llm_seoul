...
### Epic 2-1: 대화 세션 관리 (학생)
- **US-2.11** 작성하던 대화가 여러 개일 수 있고, 사이드바에서 **과거 대화 목록**으로 골라 이어 쓸 수 있다
- **US-2.12** 각 대화에는 **제목이 자동으로 붙는다**. 대화가 길어지면 갱신되고, 직접 바꾸면 그 뒤로는 덮어쓰지 않는다
- **US-2.13** 대화가 길어져도 AI가 앞부분 맥락을 잊지 않는다 (오래된 대화는 요약으로 압축되어 계속 따라온다)
- **US-2.14** 새로고침해도 대화와 선택지가 그대로 복원된다
- **US-2.15** 접수한 대화는 읽기 전용이 된다. 이어 쓰려면 새 대화를 연다 (철회해도 다시 열리지 않는다)

> **왜 대화를 여러 개 두나**: 민원 하나를 쓰다가 다른 일이 생각날 수 있고, 정보를 확인하러 갔다가
> 돌아오면 이어 써야 한다. 대화가 하나뿐이면 새로 시작할 때마다 이전 것이 사라진다.
...
### M2-1 — 대화 세션
- [ ] 새 대화를 여러 개 만들고 목록에서 골라 이어 쓸 수 있다
- [ ] 제목이 자동으로 붙고, 직접 바꾸면 자동 갱신이 덮어쓰지 않는다
- [ ] 새로고침해도 대화와 선택지가 복원된다
- [ ] 메시지가 없는 세션은 목록에 나타나지 않는다
- [ ] 접수한 대화는 읽기 전용이 되고, 철회해도 다시 열리지 않는다
- [ ] 남의 세션 id로 접근하면 404
