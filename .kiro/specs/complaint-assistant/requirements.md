...
- **US-1.2** 가입 화면에 **학교 코드 입력칸이 있다(선택)**. **비우면 학생으로 가입**되고, 채우면 그 학교에 배정된 코드와 대조해 맞으면 교직원, 틀리면 "유효하지 않은 코드"로 가입이 막힌다
  - **역할을 정하는 것은 코드 하나뿐이다.** "교직원입니다" 같은 선택을 받지 않는다 — 받으면 클라이언트가 스스로 관리자라고 주장할 수 있다. 코드는 서버만 아는 값이다
  - **틀린 코드를 조용히 학생으로 강등시키지 않는다.** 그러면 관리자로 가입된 줄 알고 관리자 화면을 찾다 헤맨다
...
- **US-2.10** 철회는 본인이 접수한 민원에만 가능하다 (다른 학생의 민원은 철회 버튼조차 보이지 않음)
  - **"내 글" 여부는 서버가 계산해 불린 하나로 내려준다.** 익명이라 작성자 id를 응답에 실을 수 없고, 그러면 클라이언트가 판단할 근거도 없다. 서버만 아는 사실이다
  - 화면에서 버튼을 감추는 것은 편의이고, **막는 것은 서버다** — 직접 호출해도 소유자가 아니면 거절된다
...
- **US-4.9** 학생이 철회한 민원은 **관리자 목록과 학생 게시판 양쪽에서** 사라진다 (별도 "철회됨" 탭은 1차 범위 밖)
  - 다만 **철회한 본인의 "과거 대화" 목록에는 남는다**(US-2.15). 그건 민원 목록이 아니라 자기 대화 기록이고, 사라지면 자기가 뭘 냈었는지 모른다
...
- [ ] 가입 폼은 학교 드롭다운(검색·별칭) · 이메일 아이디 · 비밀번호 · 학교 코드(선택)로 이뤄진다
- [ ] 학교를 고르면 도메인이 잠기고 아이디만 입력한다
- [ ] 코드를 비우고 가입하면 학생이 된다
- [ ] 그 학교 코드를 정확히 넣으면 교직원이 된다
- [ ] 틀린 코드를 넣으면 "유효하지 않은 코드"로 가입이 막힌다 (조용히 학생이 되지 않는다)
- [ ] 이미 가입된 이메일로 시도하면 중복 경고가 뜬다
...
- [ ] 정보가 부족하면 **`ask_followup` 도구**를 불러 질문과 **선택지(칩)**를 함께 돌려준다
- [ ] 충분하면 **`classify_and_refine` 도구**를 불러 카테고리(고정 목록 중 하나)/위치/제목/본문을 확정한다
- [ ] 매 턴 둘 중 하나는 반드시 불린다 (`tool_choice: any`)
- [ ] 학생이 답하면 지금까지의 맥락(요약 + 최근 대화)으로 다시 호출한다
- [ ] 칩을 누르면 그 문구가 메시지로 전송된다 (선택 전용 API가 없다)
- [ ] 같은 질문이 여러 번 반복되면 예시를 덧붙이고, 계속 맴돌면 대화를 끊고 다른 길을 제시한다
...
- [ ] 본인이 접수한 민원에만 "철회" 버튼이 보인다 (서버가 계산한 값으로 판단)
- [ ] 철회는 세 단계다 — ① 경고 + 비밀번호 ② 최종 확인창 ③ 실행
- [ ] 비밀번호가 틀리면 창을 유지한 채 경고하고 **최종 확인창으로 넘어가지 않는다**
- [ ] 맞으면 "정말 삭제하시겠습니까?"가 뜨고, 확인해야 삭제된다
- [ ] 삭제되면 "삭제되었습니다" 알림이 뜬다
- [ ] 철회 후 게시판과 관리자 목록 양쪽에서 즉시 사라진다
- [ ] 소유자가 아닌 민원을 직접 호출해도 거절된다
