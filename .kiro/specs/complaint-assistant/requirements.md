...
확인 (confirmed — 관리자가 열람함. 이 상태에서만 수락/보류/거절 버튼이 나타남)
  ...
  │                                    해결완료 (resolved) ── 최종 상태
  │
  └─→ [거절 버튼 — 코멘트 필수] → 거절 (rejected) ── 최종 상태
...
- **수락은 최종 상태가 아니라 처리 시작 신호다.** `확인 → 처리중`으로만 넘어가고, 실제로 조치가 끝나면 별도의 "해결 완료" 버튼으로 `처리중 → 해결완료`로 다시 전환해야 한다. 확인에서 곧바로 해결완료로 갈 수는 없다 (먼저 수락 또는 보류를 거쳐야 함).
- **보류도 "해결 완료" 버튼으로 `보류 → 해결완료`로 전환할 수 있다.** 보류로 세워둔 민원이 실제로 해결되면 처리중을 다시 거치지 않고 바로 해결완료로 넘긴다. 즉 해결완료로 들어오는 경로는 `처리중`과 `보류` 두 가지다.
- **보류는 코멘트 입력이 필수다.** 보류 버튼을 누르면 사유 입력창이 뜨고, 텍스트를 채워야만 실제로 `보류` 상태로 바뀐다. 빈 사유로는 보류 처리가 완료되지 않는다 (사유 없는 보류를 남기지 않도록 상태 전이와 코멘트 저장을 한 트랜잭션으로 묶는다).
- **거절도 코멘트 입력이 필수다.** 거절 버튼을 누르면 사유 입력창이 뜨고, 텍스트를 채워야만 실제로 `거절` 상태로 바뀐다. 사유 없는 거절은 남지 않는다 (보류와 동일하게 상태 전이와 사유 저장을 한 트랜잭션으로 묶는다). 거절은 최종 상태다.
- **코멘트는 상태와 무관하게 언제든 추가할 수 있다.** 보류·거절로 전환하는 순간에만 사유가 필수이고, 그 외에는 관리자가 아무 때나 코멘트를 남길 수 있다 (진행 상황 공유, 추가 설명 등). 코멘트는 여러 개 누적 가능 — 단일 필드 덮어쓰기가 아니다.
