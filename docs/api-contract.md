...
| 2.11 | 과거 대화 목록에서 골라 이어 쓰기 | #8-1 `listSessions` · #8-2 `getSession` |
| 2.12 | 제목 자동 생성·수동 고정 | 서버가 붙인다. `SessionSummary.title` |
| 2.13 | 긴 대화에서도 맥락 유지 | 서버가 압축해 들고 간다. 프론트는 모른다 |
| 2.14 | 새로고침 복원 | #8-2(단계·칩) + #10(대화) |
| 2.15 | 접수한 대화는 읽기 전용 | `SessionSummary.submitted` · `409 SESSION_CLOSED` |
