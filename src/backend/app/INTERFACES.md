...
**이 문서 + 각 폴더의 `*.py` 스텁 파일이 실제 계약이다.** 스텁 파일은 함수 시그니처와 docstring만 있고 본문은 `raise NotImplementedError`다 — 임포트는 되지만 아직 안 구현됐다는 뜻이다. 팀원은 자기 담당 폴더의 스텁을 실제 로직으로 채우면 되고, **시그니처(파라미터 이름·타입·반환 타입)는 합의 없이 바꾸지 않는다.** 바꿔야 하면 이 문서와 스텁을 먼저 고치고 알린다.
