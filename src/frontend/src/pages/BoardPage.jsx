...
export default function BoardPage() {
  ...
  const catFiltered = filter === '전체' ? complaints :
    filter === '기타' ? complaints.filter(c => !['냉난방 / 설비', '배관 / 위생', '전기 / 설비', '기자재 / 영상', '공간 / 편의'].includes(c.category)) :
    complaints.filter(c => c.category === filter);

  const filtered = catFiltered.filter(c =>
    !search || c.title.includes(search) || c.location.includes(search) || (c.summary || '').includes(search)
  );
